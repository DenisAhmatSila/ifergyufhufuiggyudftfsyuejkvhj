...
def detact_colision(dx, dy, ball, rect):
    if dx > 0:
...
while True:
    ...
    if ball.colliderect(paddle) and dy > 0:
        dx, dy = detact_colision(dx, dy, ball, paddle)
    #collision block
    hit_index = ball.collidelist(block_list)
    if hit_index != -1:
        hit_rect = block_list.pop(hit_index)
        hit_color = color_list.pop(hit_index)
        dx, dy = detact_colision(dx, dy, ball, hit_rect)
        #spcial effect
    ...
    if ball.bottom > HEIGHT:
        render_end = font_end.render('!!!!!!!лделоеошщпло!!', 1, pygame.Color('pink'))
        sc.blit(render_end, (400, 300))
    elif not len(block_list):
        render_win = font_win.render('YOU WIN!!!!!!!!!', 1, pygame.Color('pink'))
        sc.blit(render_end, (400, 300))
    #upgrade screen
    pygame.display.flip()
    clock.tick(fps)
